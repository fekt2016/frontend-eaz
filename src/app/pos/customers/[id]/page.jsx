...
export default function CustomerDetailPage() {
  ...
  if (error) return (
    <div className="text-center py-16 text-red-400">
      {error} — <Link href="/pos/customers" className="text-amber-400 hover:underline">Back</Link>
    </div>
  );

  if (!data) return (
    <div className="text-center py-16 text-gray-500">
      Customer not found. <Link href="/pos/customers" className="text-amber-400 hover:underline">Back</Link>
    </div>
  );
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      <div className="flex items-center gap-3">
        <Link href="/pos/customers" className="w-8 h-8 rounded-lg bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-400 hover:text-white transition">
          <FaArrowLeft size={12} />
        </Link>
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-bold text-white">{customer.phone}</h1>
            {activeJobs.length > 0 && (
        ...
        <Link
          href={`/pos/jobs/new?customer=${customer._id}`}
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm text-white">{new Date(customer.createdAt).toLocaleDateString("en-GH", { dateStyle: "medium" })}</p>
            </div>
          </div>
          {customer.address && (
          ...
          {customer.notes && (
            <div className="sm:col-span-3 px-3 py-2 rounded-xl bg-gray-800">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4 text-center">
          <p className="text-xl font-bold text-green-400">GH₵{totalSpent.toLocaleString()}</p>
      ...
      {devices.length > 0 && (
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {devices.map(d => (
              <span key={d} className="text-xs px-2.5 py-1 rounded-lg bg-gray-800 border border-gray-700 text-gray-300">
                {d}
              </span>
            ))}
          </div>
      ...
      <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-800">
          <h2 className="text-sm font-semibold text-white">Repair History</h2>
          {/* Filter tabs */}
            ...
            {[
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-md text-xs font-medium transition ${
                  filter === t.key ? "bg-gray-700 text-white" : "text-gray-500 hover:text-gray-300"
                }`}
              >
                {t.label}
              </button>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 min-w-0">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 ${
                        job.status === "collected" ? "bg-gray-700" :
                        job.status === "cancelled" ? "bg-red-500/10" : "bg-amber-500/10"
                      }`}>
                        {job.status === "collected" ? <FaCheckCircle size={13} className="text-gray-400" /> :
                         job.status === "cancelled" ? <FaExclamationTriangle size={12} className="text-red-400" /> :
                         <FaClock size={12} className="text-amber-400" />}
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <Link href={`/pos/jobs/${job._id}`} className="text-sm font-mono font-semibold text-amber-400 hover:underline">
                            {job.jobNumber}
                          </Link>
                          <span className={`text-xs px-2 py-0.5 rounded-full border font-medium capitalize ${STATUS_COLORS[job.status] || "bg-gray-700 text-gray-300"}`}>
                            {job.status.replace(/_/g, " ")}
                          </span>
                          {job.priority === "urgent" && (
                            <span className="text-xs text-red-400 bg-red-500/10 border border-red-500/20 px-1.5 py-0.5 rounded-full">Urgent</span>
                          )}
                        </div>
                    ...
                    <div className="text-right flex-shrink-0 space-y-1.5">
                      <p className="text-sm font-semibold text-white">GH₵{amount.toLocaleString()}</p>
