...
export default function JobDetailPage() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-48">
      <div className="w-6 h-6 border-2 border-gray-700 border-t-amber-400 rounded-full animate-spin" />
    </div>
  );

  if (!job && !loading) return (
    <div className="text-center py-16 text-gray-500">
      Job not found. <Link href="/pos/jobs" className="text-amber-400 hover:underline">Back to jobs</Link>
    </div>
  );
    ...
    <div className="max-w-4xl mx-auto space-y-5 print:p-0 print:space-y-4">
      ...
      <div className="flex items-start justify-between print:hidden">
        <div className="flex items-center gap-3">
          <Link href="/pos/jobs" className="w-8 h-8 rounded-lg bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-400 hover:text-white transition">
            <FaArrowLeft size={12} />
          </Link>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-white font-mono">{job.jobNumber}</h1>
              {job.priority === "urgent" && (
        ...
        <div className="flex gap-2">
          ...
          <button onClick={handlePrint} className="flex items-center gap-2 px-3.5 py-2 rounded-xl border border-gray-800 text-gray-400 hover:text-white hover:border-gray-700 text-sm transition">
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          {/* Customer + Device */}
          <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5 print:border print:rounded-none">
            <div className="grid sm:grid-cols-2 gap-5">
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Customer</p>
                <p className="text-sm font-semibold text-white">{job.customer?.name}</p>
                <p className="text-sm text-gray-400">{job.customer?.phone}</p>
                {job.customer?.email && <p className="text-sm text-gray-400">{job.customer.email}</p>}
              </div>
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Device</p>
                <p className="text-sm font-semibold text-white">{[job.deviceBrand, job.deviceModel].filter(Boolean).join(" ") || "—"}</p>
                <p className="text-sm text-gray-400 capitalize">{job.deviceType}</p>
            ...
            <div className="mt-4 pt-4 border-t border-gray-800">
              ...
              <p className="text-sm text-gray-300">{job.faultDescription}</p>
              {job.requiresDiagnosis && (
                <div className="mt-3 flex items-center gap-2 px-3 py-2 rounded-lg bg-purple-500/10 border border-purple-500/20">
                  <span className="text-xs text-purple-400 font-medium">Diagnosis required</span>
              ...
              {job.repairWork && (
                <div className="mt-3">
                  ...
                  <p className="text-sm text-gray-300">{job.repairWork}</p>
                </div>
              )}
              {job.estimatedCompletion && (
                <div className="mt-3 flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-500/10 border border-blue-500/20">
                  <span className="text-xs text-blue-400 font-medium">Est. completion:</span>
              ...
              {job.warrantyDays > 0 && (
                <div className={`mt-3 flex items-center justify-between px-3 py-2 rounded-lg border text-xs ${
                  job.warrantyStatus === "active"        ? "bg-green-500/10 border-green-500/20 text-green-400" :
                  job.warrantyStatus === "expiring_soon" ? "bg-amber-500/10 border-amber-500/20 text-amber-400" :
                  job.warrantyStatus === "expired"       ? "bg-gray-500/10 border-gray-700 text-gray-400" :
          ...
          {/* Technician section */}
          <div className="bg-gray-900 rounded-2xl border border-gray-800 print:hidden">
            <div className="flex items-center gap-2 px-5 py-3.5 border-b border-gray-800 bg-gray-800/50">
              ...
              <p className="text-xs font-semibold text-amber-400 uppercase tracking-wide">Technician Update</p>
            </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                {job?.requiresDiagnosis && (
                  <div>
                    <label className={labelCls}>
                      Diagnosis fee (GH₵)
                      <span className="ml-2 text-purple-400 text-xs font-normal">upfront</span>
                    </label>
                    <input
            ...
            <div className="p-5 space-y-3">
                  ...
                  <div className="absolute z-10 w-full mt-1 bg-gray-800 rounded-xl border border-gray-700 overflow-hidden shadow-xl">
                    {partResults.map(p => (
                      <button
                        ...
                        className="w-full text-left px-4 py-3 hover:bg-gray-700 transition flex items-center justify-between gap-3"
                      >
                        ...
                        <div className="text-right flex-shrink-0">
                          <p className="text-sm text-amber-400 font-semibold">GH₵{(p.sellingPrice || 0).toLocaleString()}</p>
              ...
              {selectedParts.length === 0 ? (
                ...
                <div className="space-y-2">
                  {selectedParts.map(p => (
                    <div key={p.id} className="flex items-center gap-2 p-3 rounded-xl bg-gray-800">
                      ...
                      <div className="flex items-center gap-1.5 print:hidden">
                        <button onClick={() => updatePart(p.id, "quantity", Math.max(1, p.quantity - 1))} className="w-6 h-6 rounded-md bg-gray-700 text-white text-xs flex items-center justify-center hover:bg-gray-600">−</button>
                        <span className="text-sm text-white w-5 text-center">{p.quantity}</span>
                        <button onClick={() => updatePart(p.id, "quantity", p.quantity + 1)} className="w-6 h-6 rounded-md bg-gray-700 text-white text-xs flex items-center justify-center hover:bg-gray-600">+</button>
                      </div>
                      <p className="text-sm font-semibold text-amber-400 w-20 text-right">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden print:hidden">
            <div className={`p-4 text-center border-b ${STATUS_COLORS[status] || "bg-gray-800 border-gray-700 text-gray-300"}`}>
            ...
            {status === "received" && (
              <div className="p-3 flex flex-col gap-2">
                ...
                <button onClick={() => quickStatus("repairing")} className="w-full py-2 rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs font-semibold transition">
                  Skip to Repairing
                </button>
              </div>
            )}
            ...
            {status === "ready" && !isTechnician && (
              <div className="p-3">
                <button onClick={() => quickStatus("collected")} className="w-full py-2 rounded-xl bg-gray-600 hover:bg-gray-700 text-white text-xs font-semibold transition">
            ...
            {["received","diagnosing","repairing","ready"].includes(status) && (
              <div className="px-3 pb-3">
                <button onClick={() => quickStatus("cancelled")} className="w-full py-1.5 rounded-xl border border-red-500/30 text-red-400 text-xs hover:bg-red-500/10 transition">
                  Cancel Job
                </button>
              </div>
            )}
          ...
          {/* Invoice — teller view */}
          <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden">
            <div className="px-5 py-3.5 border-b border-gray-800 bg-gray-800/50 flex items-center justify-between">
              <p className="text-xs font-semibold text-gray-300 uppercase tracking-wide">Invoice</p>
              <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${STATUS_COLORS[status] || "bg-gray-700 text-gray-300"}`}>
            ...
            {(repairWork || job?.repairWork) && (
              <div className="px-5 py-3.5 border-b border-gray-800 bg-amber-500/5">
                <p className="text-xs text-amber-400 font-medium mb-1">Repair work</p>
                <p className="text-sm text-gray-200">{repairWork || job?.repairWork}</p>
              </div>
            )}
              ...
              <div className="px-5 py-3 border-b border-gray-800 space-y-1.5">
                ...
                {selectedParts.map(p => (
                  <div key={p.id} className="flex justify-between text-xs">
                    <span className="text-gray-400">{p.name} × {p.quantity}</span>
                    <span className="text-white">GH₵{((p.cost || 0) * p.quantity).toLocaleString()}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="px-5 py-4 space-y-2.5">
              {job?.requiresDiagnosis && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-white">GH₵{(Number(diagnosisFee) || 0).toLocaleString()}</span>
                </div>
              )}
              <div className="flex justify-between text-sm">
                <span className="text-gray-400">Parts</span>
                <span className="text-white">GH₵{totalParts.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-400">Labour</span>
                <span className="text-white">GH₵{(Number(laborCost) || 0).toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-sm font-semibold border-t border-gray-800 pt-2.5">
                <span className="text-gray-300">Total</span>
                <span className="text-white">GH₵{totalAmount.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-400">Paid</span>
                <span className="text-green-400">GH₵{totalPaid.toLocaleString()}</span>
              </div>
              <div className={`flex justify-between text-base font-bold border-t border-gray-800 pt-2.5 ${balanceDue > 0 ? "text-red-400" : "text-green-400"}`}>
              ...
              {!isTechnician && totalAmount > 0 && (
                <div className="mt-3 pt-3 border-t border-gray-800 space-y-1.5">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-gray-400">GH₵{totalPartsCost.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-500">Revenue</span>
                    <span className="text-gray-300">GH₵{totalAmount.toLocaleString()}</span>
                  </div>
                  <div className={`flex justify-between text-sm font-bold pt-1 ${grossProfit >= 0 ? "text-green-400" : "text-red-400"}`}>
          ...
          <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden print:hidden">
            <div className="px-5 py-3.5 border-b border-gray-800 bg-gray-800/50">
              <p className="text-xs font-semibold text-gray-300 uppercase tracking-wide">Collect Payment</p>
            </div>
            <div className="p-5 space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {job?.requiresDiagnosis && (Number(diagnosisFee) || 0) > 0 && totalPaid < (Number(diagnosisFee) || 0) && (
                    <button type="button" onClick={() => setPayAmount(String(Number(diagnosisFee)))}
                      className="px-3 py-1.5 rounded-lg bg-purple-500/15 text-purple-400 text-xs font-medium hover:bg-purple-500/30 transition border border-purple-500/20">
                  ...
                  {(totalParts + (Number(laborCost) || 0)) > 0 && (
                    <button type="button" onClick={() => setPayAmount(String(Math.max(0, totalParts + (Number(laborCost) || 0) - Math.max(0, totalPaid - (Number(diagnosisFee) || 0)))))}
                      className="px-3 py-1.5 rounded-lg bg-amber-500/15 text-amber-400 text-xs font-medium hover:bg-amber-500/30 transition border border-amber-500/20">
                      Parts + Labour GH₵{(totalParts + (Number(laborCost) || 0)).toLocaleString()}
                    </button>
                  )}
                  {balanceDue > 0 && (
                    <button type="button" onClick={() => setPayAmount(String(balanceDue))}
                      className="px-3 py-1.5 rounded-lg bg-green-500/15 text-green-400 text-xs font-medium hover:bg-green-500/30 transition border border-green-500/20">
          ...
          <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5 space-y-4 print:hidden">
            <div className="flex items-center gap-2">
              <FaMobileAlt size={14} className="text-amber-400" />
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Request MoMo Payment</p>
            </div>

            {momoStatus === "success" ? (
              <div className="flex items-center gap-3 p-4 rounded-xl bg-green-500/10 border border-green-500/30">
                <FaCheckCircle size={20} className="text-green-400 flex-shrink-0" />
                <div>
                  <p className="text-sm font-semibold text-green-400">Payment confirmed</p>
                  <p className="text-xs text-gray-400 mt-0.5">{momoMsg}</p>
                </div>
                <button
                  onClick={cancelMomo}
              ...
              <div className="flex items-center gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/30">
                <FaTimesCircle size={20} className="text-red-400 flex-shrink-0" />
                <div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 p-4 rounded-xl bg-amber-500/10 border border-amber-500/30">
                  <FaSpinner size={16} className="text-amber-400 animate-spin flex-shrink-0" />
                  <div>
                    <p className="text-sm font-semibold text-amber-400">Waiting for customer…</p>
                    <p className="text-xs text-gray-400 mt-0.5">{momoMsg || "Customer will receive a USSD prompt on their phone."}</p>
                    {momoRef && <p className="text-xs text-gray-600 mt-0.5 font-mono">Ref: {momoRef}</p>}
                  </div>
                </div>
                <button
                  onClick={cancelMomo}
                  className="w-full py-2 rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-400 text-sm transition"
                >
                  Cancel
                </button>
              </div>
              ...
              <div className="space-y-2">
                {job.payments.map(p => (
                  <div key={p._id} className="flex items-center justify-between py-2 border-b border-gray-800 last:border-0">
                    <div>
                      <p className="text-sm text-white capitalize">{p.method}</p>
                      <p className="text-xs text-gray-500">{new Date(p.createdAt).toLocaleDateString("en-GH")}</p>
                    </div>
                    <p className="text-sm font-semibold text-green-400">GH₵{p.amount.toLocaleString()}</p>
                  </div>
                ))}
              </div>
            </div>
