...
const STATUS_COLORS = {
  ...
  repairing:         "bg-amber-500/20 text-amber-400",
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        {loading && <FaSpinner className="animate-spin text-amber-400" size={16} />}
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-2 ml-auto">
            <input
              ...
              className="px-3 py-1.5 rounded-lg bg-gray-800 border border-gray-700 text-xs text-white focus:outline-none focus:border-amber-500"
            />
      ...
      {loading && !data ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[...Array(8)].map((_, i) => <div key={i} className="h-28 rounded-2xl bg-gray-900 border border-gray-800 animate-pulse" />)}
        </div>
      ) : (
        <>
          {/* Key stats */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <Stat label="Total Revenue"     value={`GH₵${(stats?.totalRevenue || 0).toLocaleString()}`}  color="text-green-400"  icon={FaMoneyBillWave} sub={`${stats?.totalPayments || 0} payments`} />
            <Stat label="Today's Revenue"   value={`GH₵${(stats?.todayRevenue  || 0).toLocaleString()}`} color="text-amber-400"  icon={FaChartBar}      sub="Payments today" />
            <Stat label="Total Jobs"        value={stats?.totalJobs}        icon={FaWrench}    sub="All time" />
            <Stat label="New Today"         value={stats?.todayJobs}        icon={FaWrench}    sub="Jobs created today" color="text-blue-400" />
            <Stat label="In Progress"       value={stats?.pendingJobs}      icon={FaSpinner}   color="text-amber-400" sub="Active repairs" />
            <Stat label="Ready to Collect"  value={stats?.readyJobs}        icon={FaCheckCircle} color="text-green-400" sub="Awaiting pickup" />
            <Stat label="Customers"         value={stats?.totalCustomers}   icon={FaUsers}     sub="Registered" />
            <Stat label="Low Stock"         value={stats?.lowStockCount}    icon={FaExclamationTriangle} color={stats?.lowStockCount > 0 ? "text-red-400" : "text-green-400"} sub="Parts below threshold" />
            <Stat label="Total Expenses"    value={`GH₵${(stats?.totalExpenses || 0).toLocaleString()}`} color="text-red-400" icon={FaReceipt} sub="Running costs" />
            <Stat
              label="Net Profit"
              value={`GH₵${(stats?.netProfit || 0).toLocaleString()}`}
              color={(stats?.netProfit || 0) >= 0 ? "text-green-400" : "text-red-400"}
              icon={FaBalanceScale}
          ...
          {daily.length > 0 && (
            <div className="bg-gray-900 rounded-2xl border border-gray-800 p-6">
              <h2 className="text-sm font-semibold text-white mb-5">Daily Revenue</h2>
          ...
          {expenseCats.length > 0 && (
            <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/pos/expenses" className="text-xs text-amber-400 hover:underline">Manage →</Link>
              </div>
                  ...
                  return (
                    <div key={e._id}>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-gray-300 capitalize">{e._id}</span>
                        <span className="text-gray-400">GH₵{e.total.toLocaleString()} · {pct}%</span>
                      </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-800 flex justify-between text-sm">
                ...
                <span className="text-red-400 font-semibold">GH₵{(stats?.totalExpenses || 0).toLocaleString()}</span>
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-5">
                    ...
                    return (
                      <div key={m._id}>
                        <div className="flex items-center justify-between text-sm mb-1">
                          ...
                          <span className="text-white font-semibold">GH₵{m.total.toLocaleString()}</span>
                        </div>
                ...
                <div className="space-y-2">
                  {byStatus.map(s => (
                    <div key={s._id} className="flex items-center justify-between">
                      <span className={`text-xs px-2.5 py-1 rounded-full font-medium capitalize ${STATUS_COLORS[s._id] || "bg-gray-700 text-gray-300"}`}>
                        {s._id.replace(/_/g, " ")}
                      </span>
            ...
            {topParts.length > 0 && (
              <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5">
                <h2 className="text-sm font-semibold text-white mb-4">Top Parts Used</h2>
                  ...
                  {topParts.map((p, i) => {
                    ...
                    return (
                      <div key={p._id} className="flex items-center gap-3">
                        ...
                        <span className="flex-1 text-sm text-gray-300 truncate">{p._id}</span>
                        <span className="text-xs text-gray-500">{p.timesUsed}×</span>
                        <span className={`text-xs font-medium w-12 text-right ${margin >= 40 ? "text-green-400" : margin >= 20 ? "text-amber-400" : "text-red-400"}`}>{margin}%</span>
                        <span className="text-sm text-amber-400 font-semibold w-24 text-right">GH₵{p.revenue.toLocaleString()}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            {topProfitJobs.length > 0 && (
              <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5">
                <h2 className="text-sm font-semibold text-white mb-4">Most Profitable Jobs</h2>
                <div className="space-y-3">
                  {topProfitJobs.map((job, i) => {
                    const pct = job.totalRevenue > 0 ? Math.round((job.grossProfit / job.totalRevenue) * 100) : 0;
                    return (
                      <Link key={job._id} href={`/pos/jobs/${job._id}`} className="block hover:opacity-80 transition">
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-2 min-w-0">
                            <span className="text-xs text-gray-600 w-4">{i + 1}</span>
                            <span className="text-xs font-mono text-amber-400">{job.jobNumber}</span>
                            <span className="text-xs text-gray-400 truncate">{job.customer?.name}</span>
                          </div>
                          <div className="flex items-center gap-3 flex-shrink-0">
                            <span className={`text-xs font-semibold ${pct >= 50 ? "text-green-400" : pct >= 25 ? "text-amber-400" : "text-red-400"}`}>{pct}%</span>
                            <span className="text-sm font-bold text-green-400 w-24 text-right">GH₵{job.grossProfit.toLocaleString()}</span>
                          </div>
                        </div>
                        <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden">
            ...
            {techs.length > 0 && (
              <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5">
                <h2 className="text-sm font-semibold text-white mb-4">Technician Performance</h2>
                <div className="space-y-3">
                  {techs.map(t => {
          ...
          {recent.length > 0 && (
            <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-800">
                <h2 className="text-sm font-semibold text-white">Recent Repairs</h2>
              </div>
                    ...
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-mono text-amber-400">{job.jobNumber}</p>
                          <span className={`text-xs px-2 py-0.5 rounded-full capitalize ${STATUS_COLORS[job.status] || "bg-gray-700 text-gray-300"}`}>
                      ...
                      <div className="text-right">
                        <p className="text-sm font-semibold text-white">GH₵{jobTotal.toLocaleString()}</p>
