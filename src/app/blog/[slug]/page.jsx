        ...
        <ul key={`ul-${i}`} className="space-y-2 mb-4">
          {items.map((item, j) => (
            <li key={j} className="flex items-start gap-2 text-gray-600 dark:text-slate-400">
              ...
              <span dangerouslySetInnerHTML={{ __html: item.replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>').replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-amber-500 hover:underline">$1</a>') }} />
            </li>
          ))}
        </ul>
      );
    ...
    <div className="bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-white">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4 pb-6 border-b border-gray-100 dark:border-slate-800">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/30 flex items-center justify-center font-bold text-amber-500 text-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {related.map((p) => (
                <Link key={p._id} href={`/blog/${p.slug}`}
                  ...
                  <h3 className="font-semibold text-gray-900 dark:text-white text-sm leading-snug group-hover:text-amber-500 transition mb-2">{p.title}</h3>
      ...
      <section className="py-16 px-4">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <Link href="/book-consultation" className="px-6 py-3 rounded-full bg-gray-900 dark:bg-amber-500 text-white dark:text-gray-900 font-semibold hover:bg-gray-700 dark:hover:bg-amber-400 transition text-sm">
            Book a Free Consultation →
          </Link>
        </div>
      </section>
