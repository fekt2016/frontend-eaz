        ...
        <div className="max-w-md w-full text-center p-10 rounded-2xl border border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
          ...
          <div className="rounded-xl border border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 p-4 mb-5 text-left">
            ...
            <div className="flex gap-2">
              ...
              >
                Track now <FaArrowRight size={10} />
              </Link>
              <button
                type="button"
                onClick={copyLink}
              ...
              >
                {copied ? <><FaCheckCircle className="text-emerald-500" size={11}/> Copied</> : <><FaCopy size={11} /> Copy link</>}
              </button>
            </div>
          </div>
              ...
              {result.dropoff === "rider" ? (
                <>
                  <li className="flex items-center gap-2 text-xs text-gray-600 dark:text-slate-400">
                    <FaMotorcycle className="text-brand-400 flex-shrink-0" size={11} /> Our rider will call {result.customer?.phone} to arrange pickup from {result.pickupAddress || "your location"}
                  </li>
                  <li className="flex items-center gap-2 text-xs text-gray-600 dark:text-slate-400">
                    <FaMobileAlt className="text-brand-400 flex-shrink-0" size={11} /> Pack your device securely and keep your login codes handy
                  </li>
                </>
              ) : (
                <>
                  <li className="flex items-center gap-2 text-xs text-gray-600 dark:text-slate-400">
                    <FaStore className="text-brand-400 flex-shrink-0" size={11} /> Bring the device to our shop
                  </li>
                  <li className="pl-5 text-[11px] text-gray-500 dark:text-slate-500">{SHOP_ADDRESS}</li>
                  <li className="flex items-center gap-2 text-xs text-gray-600 dark:text-slate-400">
                    <FaClock className="text-brand-400 flex-shrink-0" size={11} /> Open Mon–Fri 9AM–6PM · Sat 10AM–4PM
                  </li>
                </>
              )}
              <li className="flex items-center gap-2 text-xs text-gray-600 dark:text-slate-400">
                <FaShieldAlt className="text-brand-400 flex-shrink-0" size={11} /> Free assessment — we only start work after you approve the quote
              </li>
            </ul>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              >
                {status === "loading" ? (
                  ...
                  <>Book My Repair <FaArrowRight size={12} /></>
                )}
              </button>

              <p className="text-gray-400 dark:text-slate-500 text-xs text-center">
                🔒 Your details are only used for this repair. You&apos;ll get a tracking link by SMS/email.
              </p>
            </form>
          </div>
        </div>
